...
class ControlHandler():
    ...
    def __init__(self,data,box_constraints):
        ...
        N_x = domain.grid(0).shape[0]

        t_array = np.linspace(0,data['T_final']+data['dt'],N_t)     #FIXME please!

        self.control_array = 0.1*np.ones((N_x,N_t))
    ...
    def compute_gradient(self,stateHandler,adjointHandler):
        ...
        beta            = self.data['beta']
        self.grad_array = beta*self.control_array + px_t(t_c)*u_t(t_c)
...
class AdjointHandler():
    ...
    def solve(self,u_data,control_data):
        ...
        adj_problem.parameters['K'] = self.data['K_int']
        ...
        def r_fun(*args):
            ...
            integ_1 = de.operators.integrate( px*u_field_tmp, 'x')
            u_field_tmp['g'] = u * np.sin(x+a)

            integ_2 = de.operators.integrate( px*u_field_tmp, 'x')
        ...
        adj_problem.add_equation("dt(p) - D*dx(px) = C()*px + K*V()*px + K*R() +  AdjRHS()")

        # Add auxiliary equation defining the first-order reduction
        adj_problem.add_equation("px - dx(p) = 0")

        # Build solver
        solver = adj_problem.build_solver('RK222')    # RK222
...
class StateHandler():
    ...
    def solve(self,control_data):
        ...
        state_problem.parameters['K'] = self.data['K_int']         #interaction strength
        dt      = self.data['dt']
        T_final = self.data['T_final']
        ...
        state_problem.add_equation("dt(u) - D*dx(ux) = - K*dx(v*u) - dx( C()*u )  ")

        # Build solver
        solver = state_problem.build_solver('RK222')    # RK222 #CNAB2
